# Release-Structured, Revision-Aware Real-Time Nowcasting of U.S. GDP
...
**Keywords:** GDP nowcasting, real-time data, data revisions, dynamic factor models, release calendar, forecast evaluation

**JEL codes:** C32, C53, E17
...
This paper asks whether explicitly modeling the release structure improves real-time GDP nowcasting. The analysis uses a reproducible pipeline based on real-time vintages and release-specific forecast origins. The empirical target is headline U.S. real GDP growth. The main GDP release targets are the advance release `A`, the second release `S`, the third release `T`, and a mature-vintage anchor `M`. The core evaluation focuses on `A/S/T`, while `M` anchors the measurement system and robustness checks. The revision targets are `DELTA_SA`, `DELTA_TS`, and `DELTA_MT`, where each target is the change between adjacent GDP release rounds.

The central contribution is not a claim that a complex model dominates at every point in the release cycle. The evidence is more specific and more useful. Before the advance release, the information set is still incomplete and simple bridge or standard factor benchmarks remain strong. Before the second and third releases, however, a release-structured representation of the GDP measurement ladder becomes valuable. In the frozen headline evaluation, the release-structured model is the best exact real-time model for the second release, while the revision-aware model is the best model for the third release. The same model family remains important under pandemic exclusion, post-global-financial-crisis subsamples, expanded-panel scenarios, and alternative-data-choice scenarios, although the exact winning model changes across robustness checks.

The paper is organized as follows. Section 2 positions the project in the nowcasting, real-time data, and data revision literatures. Section 3 describes the data design and real-time snapshot construction. Section 4 presents the benchmark and structured models. Section 5 defines the evaluation design. Section 6 reports the main results. Section 7 summarizes robustness checks. Section 8 discusses limitations. Section 9 concludes.

## 2. Related Literature

This paper builds on four related literatures.

First, it follows the nowcasting literature that treats mixed-frequency, ragged-edge macroeconomic data as a central empirical challenge. Giannone, Reichlin, and Small (2008) show how the real-time informational content of macroeconomic releases can be evaluated through forecast updates. Mariano and Murasawa (2003) provide the classic link between monthly latent activity and quarterly GDP. Banbura and Modugno (2014) provide the estimation reference for dynamic factor models with arbitrary missing-data patterns, which is essential for real-time panels with irregular release timing.

Second, the paper follows the real-time data literature. Croushore and Stark (2001, 2003) emphasize that macroeconomic evaluation can be misleading when revised data are used as if they were available to forecasters in real time. Koenig, Dolmas, and Piger (2003) similarly show that real-time data discipline matters for forecasting comparisons. The present project operationalizes that discipline by constructing event-level release records and snapshot panels at forecast origins.

Third, the paper connects to work on data revisions as a measurement problem. Jacobs and van Norden (2011) model data revisions as a combination of measurement error and dynamics in the underlying true value. This perspective motivates the revision-aware extension: GDP releases are treated as noisy and structured measurements of an underlying quarterly activity state rather than as interchangeable observations.

Fourth, the closest applied motivation is the recent release- and revision-aware nowcasting work by Anesti, Galvao, and Miranda-Agrippino (2022). The present paper adapts that logic to a transparent U.S. GDP pipeline, with explicit separation between exact event-time snapshots and pseudo real-time snapshots, and with robustness checks designed for applied macroeconomic journal evaluation.

## 3. Data and Real-Time Design

### 3.1 GDP Release Targets

The target variable is headline U.S. real GDP growth under the BEA release convention. The target system distinguishes four release concepts:

- `A`: advance GDP release.
- `S`: second GDP release.
- `T`: third GDP release.
- `M`: mature GDP vintage, implemented as the 12-quarter-later vintage.

The main evaluation tables focus on `A/S/T` because these are the releases faced by real-time users. The mature target `M` is retained as an anchor for the latent measurement system and for robustness analysis. Revision targets are defined as `DELTA_SA = S - A`, `DELTA_TS = T - S`, and `DELTA_MT = M - T`.

The source-of-truth build for this manuscript is the frozen artifact directory:

`outputs/frozen/submission_final`

Every number reported below is traceable to a CSV in that directory. The first reading file for the result narrative is:

`outputs/frozen/submission_final/journal_results_draft.md`

### 3.2 Monthly Indicator Panel
...
### 4.1 Release-Specific Autoregression

The autoregressive benchmark estimates separate models for each GDP release target. Lag order is selected from one to four lags by BIC in an expanding-window design. This benchmark is deliberately simple. It tests whether monthly indicators and release-structure information add value beyond the target's own history.

### 4.2 Bridge Regression
...
### 4.3 Standard Dynamic Factor Model
...
### 4.4 Release-Structured Dynamic Factor Model
...
### 4.5 Revision-Aware Dynamic Factor Model
...
This model is structural for the GDP release ladder, not for all source-data revisions. That distinction matters. The current paper asks whether GDP release revisions contain forecastable structure. It does not claim to solve the broader problem of fully modeling revisions across the entire macroeconomic indicator panel.

## 5. Forecast Evaluation Design

The evaluation sample is `2005Q1-2024Q4`, with 80 quarterly forecast targets in the headline tables. Earlier observations are used for burn-in and model estimation. The main release checkpoints are:

- `pre_advance`: day before the advance GDP release.
- `pre_second`: day before the second GDP release.
- `pre_third`: day before the third GDP release.

The full pipeline also supports earlier within-quarter checkpoints, but the manuscript focuses on the release-specific checkpoints because these are the most directly tied to the `A/S/T` release ladder.
...
### 6.1 Headline Point Forecasts

Table 1 summarizes the headline winners from the frozen point-forecast results. The full formatted table is available in `paper/tables/table_1_headline_point_forecasts.md`, and the source CSV is `outputs/frozen/submission_final/headline_point_results.csv`.

**Table 1. Headline winners by target and snapshot mode**

| Snapshot mode | Target | Checkpoint | Best model | RMSE | Interpretation |
| --- | --- | --- | --- | --- | --- |
| exact | A | pre_advance | bridge | 5.151 | Advance GDP remains difficult; the bridge benchmark is strongest. |
| exact | S | pre_second | release_dfm | 0.669 | Release-structured modeling is strongest for the second release. |
| exact | T | pre_third | revision_dfm | 0.364 | Revision-aware modeling is strongest for the third release. |
| pseudo | A | pre_advance | standard_dfm | 5.065 | A conventional factor benchmark is strongest under pseudo timing. |
| pseudo | S | pre_second | revision_dfm | 0.667 | Structured models dominate the second-release comparison. |
| pseudo | T | pre_third | revision_dfm | 0.388 | Revision-aware modeling remains strongest for the third release. |
...
### 6.2 Exact-vs-Pseudo Timing

Table 2 reports the exact-minus-pseudo RMSE gaps for the structured models. The full timing comparison is available in `paper/tables/table_2_exact_vs_pseudo.md`, and the source CSV is `outputs/frozen/submission_final/headline_exact_vs_pseudo.csv`.

**Table 2. Exact-minus-pseudo RMSE gaps for structured models**

| Target | Model | Exact RMSE | Pseudo RMSE | Exact minus pseudo |
| --- | --- | --- | --- | --- |
| A | release_dfm | 7.379 | 7.363 | 0.016 |
| A | revision_dfm | 7.309 | 7.294 | 0.015 |
| S | release_dfm | 0.669 | 0.668 | 0.000 |
| S | revision_dfm | 0.671 | 0.667 | 0.005 |
| T | release_dfm | 0.370 | 0.392 | -0.023 |
| T | revision_dfm | 0.364 | 0.388 | -0.024 |
...
### 6.3 Revision Forecasts

Table 3 reports revision-target forecasts. The full table is available in `paper/tables/table_3_revision_forecasts.md`, and the source CSV is `outputs/frozen/submission_final/headline_revision_results.csv`.

**Table 3. Revision forecasting results**

| Snapshot mode | Revision target | Checkpoint | Model | RMSE | MAE | Bias | Sign accuracy |
| --- | --- | --- | --- | --- | --- | --- | --- |
| exact | DELTA_SA | pre_advance | revision_dfm | 0.570 | 0.400 | 0.066 | 0.392 |
| pseudo | DELTA_SA | pre_advance | revision_dfm | 0.570 | 0.400 | 0.066 | 0.392 |
| exact | DELTA_TS | pre_second | revision_dfm | 0.361 | 0.238 | 0.054 | 0.405 |
| pseudo | DELTA_TS | pre_second | revision_dfm | 0.370 | 0.242 | 0.063 | 0.405 |

The revision results should be treated as supporting evidence. The model produces lower RMSE for `DELTA_TS` than for `DELTA_SA`, and exact timing helps slightly for `DELTA_TS`. However, sign accuracy is only modest. The evidence supports the presence of forecastable magnitude information in early revisions, but it does not justify a strong claim that revision directions are reliably predicted.

### 6.4 Forecast Update Figures

The paper includes two main figures from the frozen reporting pipeline.

![Figure 1. Nowcast path across release checkpoints.](figures/nowcast_path.png)

![Figure 2. Forecast update decomposition across checkpoints.](figures/forecast_update_decomposition.png)

Figure 1 is useful for showing how the nowcast evolves as the information set approaches the GDP release dates. Figure 2 is useful for explaining why the release-structured design matters: it separates forecast movements across checkpoints rather than treating the GDP target as a single static object.

## 7. Robustness

The robustness strategy is designed to protect the central claim rather than to search for a universal winner. The source files are:

- `outputs/frozen/submission_final/headline_point_subsample_robustness.csv`
- `outputs/frozen/submission_final/headline_point_scenario_robustness.csv`
- `outputs/frozen/submission_final/headline_point_small_sample_dm.csv`
- `outputs/frozen/submission_final/journal_winner_stability.csv`

Table 4 summarizes the exact-timing winners across key robustness checks. The full generated robustness tables are in `paper/tables/table_4_robustness_winners.md`, `paper/tables/appendix_a_subsample_robustness_full.md`, and `paper/tables/appendix_a_scenario_robustness_full.md`.

**Table 4. Exact-timing robustness winners**

| Robustness check | A winner | S winner | T winner | Interpretation |
| --- | --- | --- | --- | --- |
| Alternative data choice | bridge | release_dfm | revision_dfm | Structured models remain best for `S/T`. |
| Expanded panel | standard_dfm | revision_dfm | release_dfm | Structured model family remains best for `S/T`. |
| Exclude pandemic quarters | bridge | release_dfm | revision_dfm | The main `S/T` pattern survives pandemic exclusion. |
| Post-GFC sample | bridge | release_dfm | revision_dfm | The main `S/T` pattern survives the larger post-GFC subsample. |
| Pre-GFC sample | release_dfm | release_dfm | release_dfm | Results are supportive but based on only 12 forecasts. |

The robustness evidence supports a model-family claim, not a single-model dominance claim. The winner-stability file shows that `winner_is_stable` is false for every target and snapshot-mode pair. This instability is not fatal because the paper's main claim is not that `revision_dfm` always wins. The defensible claim is that structured models are consistently important for the second and third releases, while the advance release is harder and remains highly sensitive to model choice and sample design.
...
Anesti, N., Galvao, A. B., and Miranda-Agrippino, S. (2022). Uncertain Kingdom: Nowcasting GDP and its revisions. *Journal of Applied Econometrics*, 37(1), 42-62.
...
Croushore, D., and Stark, T. (2003). A real-time data set for macroeconomists: Does data vintage matter? *Review of Economics and Statistics*, 85(3), 605-617.

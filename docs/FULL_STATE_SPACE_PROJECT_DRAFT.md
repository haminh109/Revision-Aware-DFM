...
3. Add density forecast scoring using log score, CRPS, and interval coverage.
4. Add a clearer small-sample forecast-comparison section, ideally with conservative Diebold-Mariano or Clark-West style diagnostics where appropriate.
5. Write the paper only from a frozen output folder, not from ad hoc intermediate outputs.
